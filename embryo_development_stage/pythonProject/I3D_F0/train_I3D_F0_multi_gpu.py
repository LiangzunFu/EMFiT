...
from pythonProject.dataset import Embryo_Dataset_Time_Lapse_I3D_F0
from pythonProject.I3D.I3D_model import i3_res50
...
def main(args):
    ...
    train_dataset = Embryo_Dataset_Time_Lapse_I3D_F0(folder_path, (1,324), transform=data_transform["train"])

    train_num = len(train_dataset)
    train_class_num = train_dataset.get_class_distribution()

    test_dataset = Embryo_Dataset_Time_Lapse_I3D_F0(folder_path, video_num=(325,424), transform=data_transform["test"])
...
if __name__ == '__main__':
    ...
    parse.add_argument('--txt_path', type=str, default="/path/to/output_file_I3D_F0_gpus_new_dataset.txt")
    # the path to save every class testing accuracy
    parse.add_argument('--class_txt_path', type=str,
                       default="/path/to/output_file_I3D_F0_gpus_confusion_matrix_new_dataset.txt")
    # the path to save the .pth weight file
    parse.add_argument('--save_path', type=str, default='/path/to/weights_I3D_F0_gpus_new_dataset.pth')
    # the path to save the last epoch .pth weight file
    parse.add_argument('--save_path_last', type=str,
                       default='/path/to/weights_I3D_F0_gpus_last_new_dataset.pth')
    ...
    torchrun --nproc_per_node=2 /path/to/your/train_I3D_F0_multi_gpu.py
