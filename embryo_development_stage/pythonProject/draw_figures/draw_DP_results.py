...
from pythonProject.I3D_ViT_multi_task_loss.I3D_ViT_multi_task_loss_model import I3D_ViT_Multi_Task
...
def main():
    ...
    model.eval()
    for video_number in range(451, 475):
        all_test_num = 0
        all_acc = 0
        all_acc_dp = 0
        ...
        with torch.no_grad():
            ...
            all_predict_class = torch.tensor([]).to(device)
            lable_dp = torch.tensor([]).to(device)
            test_dataset = Embryo_Dataset_Time_Lapse_I3D_ViT_Multi_task(folder_path, video_num=(video_number, video_number),
            ...
            for test_data in test_bar:
                ...
                all_predict_class = torch.cat((all_predict_class,predict_class),dim=0)
            ...
            predicted_sequence.reverse()
            all_predict_class_list = all_predict_class.cpu().tolist()
            lable_dp_list = lable_dp.cpu().tolist()
            ...
            frames = list(range(test_num))

            stage_labels = ["tPB2", "tPNa", "tPNf", "t2", "t3", "t4", "t5", "t6", "t7", "t8", "t9+", "tM", "tSB", "tB", "tEB"]
            # 绘图
            plt.figure(figsize=(8, 6), dpi=400)
            plt.plot(frames, lable_dp_list, label='Ground truth', linewidth=3, color='royalblue')
            plt.plot(frames, all_predict_class_list, label='Raw prediction', linewidth=1, color='green')
            plt.plot(frames, predicted_sequence, label='Post processed with DP', linewidth=2, color='red')

            plt.yticks(ticks=range(len(stage_labels)), labels=stage_labels)
            plt.xlabel('Frames')
            plt.ylabel('Stage')
            plt.legend()
            plt.tight_layout()
